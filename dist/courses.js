...
function createProductCard(product) {
    ...
    if (user && user.role === 'student') {
        ...
        addToCartButton.addEventListener('click', () => addToCart(product.courseName));
...
function initCoursesPage() {
    ...
    if (enrollButton) {
        enrollButton.addEventListener('click', () => __awaiter(this, void 0, void 0, function* () {
            try {
                const userId = getCurrentUser().id;
                const courseId = 1; // Replace with the actual course ID
                yield enrollCourse(userId, courseId);
            }
            catch (error) {
